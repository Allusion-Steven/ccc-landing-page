<script lang="ts">
    ...
    const handleContinue = () => {
        ...
        goto(`/booking/${selectedVehicle}?pickup=${pickupDate}&dropoff=${dropoffDate}&location=${location}`);
    };
</script>

<div class="container mx-auto min-h-screen px-4 py-8">
    <div class="mx-auto max-w-4xl">
        <h1 class="mb-8 text-3xl font-bold text-white text-center">Book Your Dream Car</h1>
                    ...
                    >
                        ...
                        {#each vehicles as vehicle}
                            <option value={vehicle.id} class="bg-gray-800">
                                {vehicle.year} {vehicle.make} {vehicle.model} - ${vehicle.price}/day
                            </option>
                        {/each}
                    </select>
                </div>
